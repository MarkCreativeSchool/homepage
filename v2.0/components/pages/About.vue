<template>
  <section id="about">
    <div class="container">
      <div class="columns">
        <div class="column is-6">
          <div class="rare-box"></div>
          <img src="/images/about.jpg" class="about-img-small position-relative w-100" alt="教室の特徴" />
        </div>
        <div class="column is-6">
          <div class="main-title text-lg-left offset-md-1 mb-0 wow fadeInUp" data-wow-delay="300ms">
            <h5 class="wow fadeInUp" data-wow-delay="300ms">教室の<span class="color-blue">特徴</span></h5>
            <!--
            <h2 class="wow fadeInUp font-weight-light" data-wow-delay="400ms">
              必要なものは <span class="color-pink">楽しむ</span> 気持ちとアイデアだけ
            </h2>
            -->
            <h2 class="wow fadeInUp font-weight-light" data-wow-delay="400ms">
              小4から始めるつくば市のプログラミングスクール
            </h2>
            <p class="pb-4 wow fadeInUp" data-wow-delay="500ms">
              <span class="color-pink">マインクラフト</span>をベースに<span class="color-pink">競技プログラミング</span>や
              <span class="color-pink">Webアプリケーション</span>などクリエイティブなデジタルコンテンツ制作のお手伝いをしています
            </p>
            <ul class="pb-5 text-left wow fadeInUp" data-wow-delay="600ms">
              <li>小4〜高校生まで幅広い年齢のお子様が通っています</li>
              <li>Pythonなどのプログラミング言語を学習します</li>
              <li>教室・オンライン併用も可能</li>
            </ul>
            <!--
            <a href="javascript:void(0)" class="btn-setting color-black btn-hvr-up btn-yellow btn-hvr-pink text-white link wow fadeInUp" data-wow-delay="700ms">learn more</a>
            -->
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<script>
export default {
  name: "About",
}
</script>
